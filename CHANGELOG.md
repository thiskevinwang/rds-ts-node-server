...
- no more `Limit` on `getUsers` query

## [v0.7.1] - 2020-12-14

### Changed

- removed fields from User type and input
- put Discussion-related queries/mutations behind `@auth`
...
- mutation args from `String!` to `ID!`
- updateAvatarUrl returns `id` now

## [v0.6.1] - 2020-12-06

### Fixed

- missing/forgotten `id` field on `User`
  - this enables the `@apollo/client` cache to _work properly_
  - the cache typically has `ROOT_QUERY` & `ROOT_MUTATION`
  - adding `id` includes additional cache entries like `User:d5d58dad-f80d-4bce-9d15-76c59d475dd2`
    - Format: `${__typename}:${id}`

### **TODO**

- verify that the `ID!` type (vs `String!`) has an effect on the client-cache, or not
...
- updateAvatarUrl Mutation

### Changed

- s3GetSignedPutObjectUrl from Query to Mutation
- Mutation.updateUsername param type

### Removed

- getOrCreateUser dead SK conditionExpression

## [v0.5.1] - 2020-12-01

### Fixed

- remove unintentional `@development` directive from `getUsers` Query
...
[v0.5.1]: https://github.com/thiskevinwang/rds-ts-node-server/compare/v0.5.0...v0.5.1
